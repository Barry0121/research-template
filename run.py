...
logfile_fp = Path(os.path.join("exps", f"n={args.n}_aux={args.aux}_freeze={args.freeze_cnn}"))
if not os.path.exists(logfile_fp):
    os.makedirs(logfile_fp)
logging.basicConfig(level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers = [
                        logging.FileHandler(filename=logfile_fp / "logfile.log",
                                            mode='a'),
                        logging.StreamHandler()
                    ])

# Instantiate dataset, model, and optimizer (TODO: Change them accordingly)
class Example_Model(nn.Module):
    def __init__(self, n, aux, max_iter, eps, prox_lam, weight_normalize, freeze):
        super().__init__()
        self.mn = mixnet.MixNet(n=n, aux=aux, max_iter=max_iter, eps=eps, prox_lam=prox_lam, weight_normalize=weight_normalize)
...
model = Example_Model(args.n, args.aux, args.max_iter, args.eps, args.prox_lam, args.weight_normalize, args.freeze_cnn)
...
# TODO: Change save paths accordingly
